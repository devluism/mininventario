...
@section('content')
    <div class="container">
        <div class="row my-4 align-items-center">
            <div class="col-sm-9">
                <h2 class="m-0 fw-semibold">Sistema de Inventario Automatizado</h2>
                <p class="text-muted">Dashboard Administrativo - <span class="fw-semibold"><?= date('d/m/Y') ?></span></p>
            </div>

            <div class="col-sm-3">
                <label class="form-text" for="filterSelect">Seleccionar por {{ $filter->object }}</label>
                <select class="form-select" id="filterSelect"></select>
            </div>
        </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center bg-dark text-white">
                        <div class="card-title m-0 fs-6 text-muted text-white">
                            <i class="bi bi-table me-2 text-white"></i>
                            <span id="cardTitle" class="fw-semibold text-white">{{ __('Inventario de Proyectos') }}</span>
                        </div>

                        @if (Auth::user()->role == 0)
                        <div class="card-tools">
                            <button onclick="openProductModal()" class="btn btn-sm btn-primary"><i class="fas fa-plus me-2"></i>Nuevo Producto</button>
                        </div>
                        @endif
                    </div>
                            ...
                            <table id="productsTable" class="table table-striped table-bordered table-hover w-100">
                                {{-- <thead>
                                    <tr>
                                        ...
                                        <th class="text-start">Almacen</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <th class="text-center">{{ number_format($product->stock, 0, ',', '.') }}</th>
                                        <th class="text-start">{{ $product->warehouse->description }}</th>
                                        <th class="text-start">{{ $product->supplier->name }}</th>
                                        @if (Auth::user()->role == 0)                                           
                                        <th class="text-center">
                                            <div class="d-flex flex-row justify-content-evenly">
                                                <a class="btn btn-icon btn-outline-success" href="{{ url("/product/incoming/$product->id") }}"><i class="fas fa-arrow-up"></i></a>
                                                <a class="btn btn-icon btn-outline-danger" href="{{ url("/product/outgoing/$product->id") }}"><i class="fas fa-arrow-down"></i></a>
                                            </div>
                                        </th>
                                        @endif
                                    </tr>                                        
    ...
    @if (Auth::user()->role == 0)
    <div class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" id="productModal" tabindex="-1" aria-labelledby="productModalTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-fullscreen-md-down">
            <div class="modal-content p-1" id="productContent">
            </div>
        </div>
    </div>
    <div class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" id="incomingModal" tabindex="-1" aria-labelledby="incomingModalTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-fullscreen-md-down">
            <div class="modal-content p-1" id="incomingContent">
            </div>
        </div>
    </div>
    <div class="modal fade" data-bs-backdrop="static" data-bs-keyboard="false" id="outgoingModal" tabindex="-1" aria-labelledby="outgoingModalTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-fullscreen-md-down">
            <div class="modal-content p-1" id="outgoingContent">
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button id="deleteProductBtn" onclick="deleteProduct(this.value)" type="button" class="btn btn-danger" data-bs-dismiss="modal"><i class="fas fa-trash me-2"></i>Eliminar producto</button>
            </div>
          </div>
        </div>
    </div>
...
@section('scripts')
    <script>
        ...
        function openIncomingModal(id) {
            axios.get(`/product/modal/incoming/${id}`)
            .then(res => {
                $('#incomingContent').html(res.data);
                new Modal('#incomingModal').show();
            })
            .catch(err => {
        ...
        function openOutgoingModal(id) {
            axios.get(`/product/modal/outgoing/${id}`)
            .then(res => {
                $('#outgoingContent').html(res.data);
                new Modal('#outgoingModal').show();
            })
            .catch(err => {
                console.log("Error: "+err)
            })
        ...
        function deleteProduct(id) {
            axios.delete('/product/delete', { data: { id: id } })
            .then((res) => {
                $('#productsTable').DataTable().ajax.url("{{ route('product.datatable') }}").load();
                toast(res.data.status, res.data.icon, res.data.message);
            })
            .catch((err) => {
        ...
        function openObjectModal(object) {
            axios.get(`/${object}/modal/create`)
            .then(res => {
                $(`#${object}Content`).html(res.data)
                new Modal(`#${object}Modal`).show()
            })
            .catch(err => {
                console.log("Error: "+err)
            })
        ...
        function openProductModal() {
            axios.get(`/product/modal/create`)
            .then(res => {
                $('#productContent').html(res.data)
                new Modal('#productModal').show()
            })
            .catch(err => {
                console.log("Error: "+err)
            })
        ...
        function setOptions(data, type, filter=false) { 
            let options = (filter) ? '<option value="0" selected>Todos</option>' : '<option value="" selected disabled></option>';
            let text = ''
            ...
            data.forEach(object => {
                ...
                options += `
                <option value="${object.id}">${text}</option>
                `
            });
        ...
        function getFilterOptions() {
            let type = (role) ? 'project' : 'user';
            axios.get(`/${type}/list`)
            .then((res) => {
                $('#filterSelect').html(setOptions(res.data, type, true))
            })
            .catch((err) => {
        ...
        function getProjects() {
            ...
            .then((res) => {
                $('#projectSelect').html(setOptions(res.data, 'project'))
            })
            .catch((err) => {
                console.log(err.message)
            })
        ...
        function createProject() {
            let project_title = $('#createProjectForm #titleInput').val()
            let url           = $('#createProjectForm #titleInput').val()
            let user_id       = $('#createProjectForm #userSelect').val()

            axios.post('/project/create', {title: project_title, url: url, user_id:user_id})
            .then((res) => {
                toast(res.data.status, res.data.icon, res.data.message);
                getProjects();
                getFilterOptions();
            ...
            $('#closeProjectModalBtn').click();
        }

        function getUsers() {
            axios.get('/user/list')
            .then((res) => {
                $('#userSelect').html(setOptions(res.data, 'user'))
            })
            .catch((err) => {
                console.log(err)
            })
        ...
        function createUser() {
            let username   = $('#createUserForm #usernameInput').val()
            let first_name = $('#createUserForm #firstNameInput').val()
            let last_name  = $('#createUserForm #lastNameInput').val()
            let password   = $('#createUserForm #passwordInput').val()
            let rpassword  = $('#createUserForm #rPasswordInput').val()
            let email      = $('#createUserForm #emailInput').val()
            let role       = $('#createUserForm #roleInput').val()
            if (password != rpassword) {
                toast('warning', 'exclamation', 'Las contraseñas no coinciden');
                return false
            }

            axios.post('/user/create', {name: name, code: code})
            .then((res) => {
                toast(res.data.status, res.data.icon, res.data.message);
                getSupplier();
                getFilterOptions();
        ...
        function getSuppliers() {
            ...
            .then((res) => {
                $('#supplierSelect').html(setOptions(res.data, 'supplier'))
            })
            .catch((err) => {
                console.log(err)
            })
        ...
        function createSupplier() {
            let name = $('#createSupplierForm #nameInput').val()
            let code = $('#createSupplierForm #codeInput').val()

            axios.post('/supplier/create', {name: name, code: code})
            .then((res) => {
                toast(res.data.status, res.data.icon, res.data.message);
                getSupplier();
            })
            ...
            $('#closeSupplierModalBtn').click();
        }

        function getWarehouses() {
            axios.get('/warehouse/list')
            .then((res) => {
                $('#warehouseSelect').html(setOptions(res.data, 'warehouse'))
            })
            .catch((err) => {
                console.log(err)
            })
        ...
        function createWarehouse() {
            let name = $('#createWarehouseForm #nameInput').val()
            let code = $('#createWarehouseForm #codeInput').val()

            axios.post('/warehouse/create', {name: name, code: code})
            .then((res) => {
                toast(res.data.status, res.data.icon, res.data.message);
                getWarehouses();
            })
            ...
            $('#closeWarehouseModalBtn').click();
        }
    </script>

    <script type="module">
        var title = "Inventario de Proyectos";        

        $(document).ready(function() {
            getFilterOptions();
            var groupColumn = (role) ? 1 : 0;
            var table = $('#productsTable').DataTable({
                ...
                processing: true,
                serverSide: true,
                dom: "<'mb-3 d-flex flex-sm-row gap-2 align-items-center'<'col-12 col-sm-auto'l><'col-sm-2'f><'col-12 col-sm-auto ms-sm-auto'B>>" + "rt" + "<'d-flex justify-content-end'p>",
                ...
                fnCreatedRow: function(elemt, data, iDataIndex) {
                    if (!role) {
                        $('td:eq(5)', elemt).addClass('d-flex flex-row justify-content-center gap-1')
                        $('td:eq(5)', elemt).html(`
                            <button onclick="openIncomingModal(${+data.id})" class="btn btn-icon btn-sm btn-success" style="width: 80px;">Entrada<i class="fas fa-arrow-up ms-1" ></i></button>
                            <button onclick="openOutgoingModal(${+data.id})" class="btn btn-icon btn-sm btn-dark" style="width: 80px;">Salida<i class="fas fa-arrow-down ms-1"></i></button>
                        `)
                        $('td:eq(6)', elemt).html(`
                            <button onclick="$('#deleteProductBtn').val(${+data.id})" data-bs-toggle="modal" data-bs-target="#deleteProductModal" class="btn btn-icon btn-sm btn-danger"><i class="fas fa-trash"></i></button>
                        `)
                    }
                },
                buttons: [
                    {
                        ...
                        titleAttr: 'Copiar',
                        title: title,
                        className: 'btn btn-icon btn-outline-gray bg-light text-muted rounded-1 me-1',
                        exportOptions: {
                    ...
                    },
                    {
                        ...
                        customize: function(doc) {
                            ...
                            // var logo = "data:image/jpeg;base64,/9j/4RbYWx" // A documentation reference can be found at
                    ...
                    },
                    {
                        ...
                        titleAttr: 'Colvis',
                        title: title,
                        className: 'btn btn-icon btn-outline-gray bg-light text-muted rounded-1',
                        exportOptions: {
                            columns: ':not(:last-child)'
                        }
                    },
                        ...
                        .each(function (group, i) {
                            if (last !== group) {
                                $(rows)
                                    ...
                                    .before(
                                        `<tr class="group">
                                            <td class="group fw-bold" colspan="7">
                                                ${group}
                                            </td>
                                        </tr>`
                                    );
            ...
            $('#productsTable tbody').on('click', 'tr.group', function () {
        ...
        $('#filterSelect').on('change', (event) => {
            $('#productsTable').DataTable().ajax.url(`{{ url('/product/datatable') }}?${(role) ? 'project_id' : 'user_id'}=${$(event.target).val()}`).load();
            
            title = ($(event.target).val() == 0)
            ? 'Inventario de Proyectos'
            : `Inventario de Proyecto${(role) ? '' : 's de'} ${$('#filterSelect option:selected').text()}`;
