...
@section('content')
    {{-- @if (Auth::user())
        <div class="alert alert-sm alert-info position-fixed top-10 end-0 me-4 z-3" role="alert">
            <span><i class="fas fa-circle-info me-2"></i>Bienvenido {{ Auth::user()->username }}!</span>
            <button id="alertBtn" class="btn-close shadow-none ms-2" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    @endif --}}
    <div class="container">
        <div class="row my-4 align-items-center">
            <div class="col-sm-9">
                <h2 class="m-0 fw-semibold">Dashboard Administrativo</h2>
                <p class="text-muted">Sistema de inventario - <span class="fw-semibold"><?= date('d/m/Y') ?></span></p>
            </div>

            <div class="col-sm-3">
                <label class="form-text" for="projectSelect">Seleccionar proyecto</label>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div class="card-title m-0 fs-5 text-muted">
                            <i class="bi bi-table me-2"></i>
                            <span id="cardTitle" class="fw-semibold">{{ __('Inventario de Proyectos') }}</span>
                        </div>

                        <div class="card-tools">
                            <button class="btn btn-sm btn-primary"><i class="fas fa-plus me-2"></i>Nuevo Producto</button>
                        </div>
                    </div>
    ...
    <div class="modal fade text-start" id="incomingModal" tabindex="-1" aria-labelledby="incomingModalTitle"
        aria-hidden="true">
    ...
    <div class="modal fade text-start" id="outgoingModal" tabindex="-1" aria-labelledby="outgoingModalTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
...
@section('scripts')
    <script>
        const incoming = (product_id=null) => {
            alert(product_id)
        }

        const outgoing = (product_id) => {
            alert(product_id)
        }

        const openIncomingModal = (id) => {
    ...
    <script type="module">
        ...
        var table = $('#productsTable').DataTable({
            ...
            fnCreatedRow: function(elemt, data, iDataIndex) {
                $('td:eq(5)', elemt).addClass('d-flex flex-row gap-2 justify-content-around')
                $('td:eq(5)', elemt).html(`
                    <button onclick="openIncomingModal(${+data.id})" class="btn btn-icon btn-sm btn-success"><i class="fas fa-arrow-up" ></i></button>
                    <button onclick="openOutgoingModal(${+data.id})" class="btn btn-icon btn-sm btn-danger"><i class="fas fa-arrow-down"></i></button>
                `)
            },
            buttons: [
                {
        ...
        }).buttons().container().appendTo('#registrosTable_wrapper .col-md-6:eq(0)')
            
        $('#productsTable td').css('vertical-align', 'middle')
        
        $('#projectSelect').on('change', (event) => {
            $('#productsTable').DataTable().ajax.url(`{{ url('/dashboard/filter') }}/${$(event.target).val()}`).load();
        })
